...
def audit_elective_requirement(
    ...
    matched = [
        ...
        for course_code in matched
        if (
            course_credit_hours(course_code)
            is not None
            and course_credit_hours(course_code)
    ...
    # Each catalog elective requirement row represents one
    # course slot. No-double-counting is enforced by the caller's
    # used_courses set.
...
def audit_student_credential(
    ...
    # REQUIREMENT_ALLOCATION_PRIORITY_V1
    #
    # Greedy course allocation must resolve narrow requirements before
    # broad requirements. Otherwise a core bucket or elective can consume
    # the only course capable of satisfying a later exact requirement.
    grouped = list(
    ...
    def requirement_priority(item):
        ...
        course_option_count = len(course_options)

        is_elective = (
            rule_type == "ELECTIVE"
            or (
                option_types
                and option_types <= {"ELECTIVE"}
            )
        )
