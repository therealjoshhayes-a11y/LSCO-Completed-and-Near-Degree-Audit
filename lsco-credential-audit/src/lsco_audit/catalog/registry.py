...
class CatalogRecord:
    ...
    def source_pdf_path(self) -> Path:
        return self.raw_catalog_dir / self.source_pdf

    @property
    def source_docx_path(self) -> Path:
        return self.raw_catalog_dir / self.source_docx
...
def load_catalog_registry(
    path: str | Path = "config/catalogs.csv",
    ...
    if not registry_path.exists():
        raise FileNotFoundError(f"Catalog registry not found: {registry_path}")
...
def catalog_years(
    ...
    return [
        record.catalog_year
        for record in load_catalog_registry(path, active_only=active_only)
    ]
