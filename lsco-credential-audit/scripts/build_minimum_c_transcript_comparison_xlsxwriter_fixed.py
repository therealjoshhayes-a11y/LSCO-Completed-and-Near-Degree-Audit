...
def excel_safe(value):
    """Convert NaN/NaT/Inf values to blanks before XlsxWriter writes."""
    if pd.isna(value):
        return ""
...
def excel_safe_row(values):
    return [excel_safe(value) for value in values]
...
with pd.ExcelWriter(OUTPUT, engine="xlsxwriter") as writer:
    ...
    for i, field in enumerate(summary_fields, start=3):
        ...
        summary_sheet.write(i, 1, excel_safe(clean(review_award.get(field))), body_fmt)
        summary_sheet.write(i, 2, excel_safe(clean(completer_award.get(field))), body_fmt)
    ...
    for idx in range(max_rows):
        if idx < len(review_transcript):
            trans_sheet.write_row(
                idx + 3,
                0,
                excel_safe_row(review_transcript.iloc[idx].tolist()),
                body_fmt,
            )
        if idx < len(completer_transcript):
            trans_sheet.write_row(
                idx + 3,
                9,
                excel_safe_row(completer_transcript.iloc[idx].tolist()),
                body_fmt,
            )
    ...
    for idx in range(max_applied):
        if idx < len(review_applied):
            row = excel_safe_row(review_applied.iloc[idx].tolist())
            fmt = (
                pass_fmt
        ...
        if idx < len(completer_applied):
            row = excel_safe_row(completer_applied.iloc[idx].tolist())
            fmt = (
                pass_fmt
                if str(row[6]).upper() == "TRUE"
                else body_fmt
