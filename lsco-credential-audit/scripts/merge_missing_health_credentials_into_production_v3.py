...
layer, and run rollback-protected integrity tests (v3).
...
def validate_rebuilt_audit(
    ...
    # The executable master historically contains repeated requirement IDs after
    # option explosion. Do not reject pre-existing non-target multiplicity merely
    # because a display-level key looks duplicated. Instead:
    #   1. require no exact duplicate target rows using every available column;
    #   2. prove the complete non-target executable dataset is unchanged by hash.
    target_exact_dupes = target.duplicated(keep=False)
    if target_exact_dupes.any():
        sample_columns = [
            column
            for column in (
                "requirement_id",
        ...
        sample = target.loc[target_exact_dupes, sample_columns].head(20)
        fail(
            "Rebuilt target credentials contain exact duplicate executable rows:\n"
...
def main() -> int:
    ...
    try:
        # Build a fresh executable baseline from the current pre-merge source master.
        # The checked-in multiyear CSV may be stale relative to the current builder,
        # so comparing against it can falsely report unrelated changes.
        ...
        run_checked(
            ...
            root,
            "fresh pre-merge executable baseline rebuild",
        )
        audit_before = read_csv(audit_master_path)
        baseline_snapshot = backup_dir / "requirements_master_multiyear_fresh_baseline.csv"
        shutil.copy2(audit_master_path, baseline_snapshot)
        ...
        run_checked(
            [sys.executable, str(audit_builder)],
            root,
            "post-merge multiyear executable-requirements rebuild",
        )
        ...
        report = {
            ...
            "fresh_premerge_baseline": str(baseline_snapshot),
        ...
        print(f"Fresh pre-merge baseline:           {baseline_snapshot}")
