...
# Exact Livingston major-to-credential crosswalk. Titles are display values only;
# credential identity is controlled by the generated 2025-2026 credential_id.
EXACT_MAJOR_CREDENTIALS = {
    "CJCC": "CRIMINAL_JUSTICE_LAW_ENFORCEMENT_2025",
    "CJCR": "CRIMINAL_JUSTICE_AAS_2025",
    "ASMB": "BUSINESS_ADMINISTRATION_2025",
    "COSD": "COSMETOLOGY_OPERATOR_MANAGEMENT_2025",
    "COSC": "COMPUTER_SCIENCE_2025",
    "ASMD": "ANIMAL_SCIENCE_MANAGEMENT_2025",
    "ASNS": "NATURAL_SCIENCE_2025",
    "ASPM": "PRE_PROFESSIONAL_HEALTH_SCIENCE_2025",
    "ACSD": "ACCOUNTING_SERVICES_MANAGEMENT_2025",
    "AATP": "TEACHER_EDUCATION_AAS_2025",
    "AAT2": "TEACHER_EDUCATION_AAS_2025",
    "ITHC": "INFORMATION_TECHNOLOGY_SUPPORT_SPECIALIST_2025",
    "TEPC": "TEACHER_EDUCATION_CERTIFICATE_OF_COMPLETION_2025",
    "BMRD": "REAL_ESTATE_MANAGEMENT_2025",
    "WLDB": "PRODUCTION_WELDER_2025",
    # Physical Education is treated provisionally as the Liberal Arts transfer plan.
    "ASPE": "LIBERAL_ARTS_2025",
    # This plan is absent from the current requirements master but is explicitly
    # present in the 2025-2026 catalog. It is evaluated locally below.
    "VNSG": "VOCATIONAL_NURSING_2025",
}

VOCATIONAL_NURSING_OVERLAY = [
    (1, "Prerequisites", "BIOL 2401", 4.0, "REQUIRED COURSE"),
    (2, "Prerequisites", "BIOL 2402", 4.0, "REQUIRED COURSE"),
    (3, "First Semester – LEVEL I", "VNSG 1227", 2.0, "REQUIRED COURSE"),
    (4, "First Semester – LEVEL I", "VNSG 1122", 1.0, "REQUIRED COURSE"),
    (5, "First Semester – LEVEL I", "VNSG 1323", 3.0, "REQUIRED COURSE"),
    (6, "First Semester – LEVEL I", "VNSG 1160", 1.0, "REQUIRED COURSE"),
    (7, "First Semester – LEVEL I", "VNSG 1402", 4.0, "REQUIRED COURSE"),
    (8, "First Semester – LEVEL I", "VNSG 1461", 4.0, "REQUIRED COURSE"),
    (9, "Second Semester – LEVEL II", "BIOL 1322 OR FDNS 1345", 3.0, "CHOOSE FROM OPTIONS"),
    (10, "Second Semester – LEVEL II", "VNSG 1231", 2.0, "REQUIRED COURSE"),
    (11, "Second Semester – LEVEL II", "VNSG 1138", 1.0, "REQUIRED COURSE"),
    (12, "Second Semester – LEVEL II", "VNSG 1136", 1.0, "REQUIRED COURSE"),
    (13, "Second Semester – LEVEL II", "VNSG 1462", 4.0, "REQUIRED COURSE"),
    (14, "Second Semester – LEVEL II", "VNSG 2413", 4.0, "REQUIRED COURSE"),
    (15, "Third Semester – LEVEL III", "PSYC 2314", 3.0, "REQUIRED COURSE"),
    (16, "Third Semester – LEVEL III", "VNSG 1230", 2.0, "REQUIRED COURSE"),
    (17, "Third Semester – LEVEL III", "VNSG 1234", 2.0, "REQUIRED COURSE"),
    (18, "Third Semester – LEVEL III", "VNSG 2214", 2.0, "REQUIRED COURSE"),
    (19, "Third Semester – LEVEL III", "VNSG 2463", 4.0, "REQUIRED COURSE"),
]
...
def choose_credential(
    ...
    exact_id = EXACT_MAJOR_CREDENTIALS.get(major_code)

    if exact_id:
        exact_match = credential_inventory[
            credential_inventory["credential_id"].eq(exact_id)
        ]

        if not exact_match.empty:
            candidate = exact_match.iloc[0]
            progress = student_summary[
                student_summary["student_id"].eq(student_id)
                & student_summary["credential_id"].eq(exact_id)
            ]
            if progress.empty:
                met = 0.0
                total = float(candidate.get("requirement_count", 0) or 0)
                audit_status = ""
            else:
                p = progress.iloc[0]
                met = pd.to_numeric(pd.Series([p.get("requirements_met")]), errors="coerce").iloc[0]
                total = pd.to_numeric(pd.Series([p.get("requirements_total")]), errors="coerce").iloc[0]
                met = 0.0 if pd.isna(met) else float(met)
                total = 0.0 if pd.isna(total) else float(total)
                audit_status = text(p.get("audit_status"))
            ratio = met / total if total else 0.0
            return {
                "mapping_status": "MAPPED",
                "mapping_basis": "EXACT_CROSSWALK",
                "credential_id": exact_id,
                "credential_title": candidate["credential_title"],
        ...
        if major_code == "VNSG":
            return {
                "mapping_status": "MAPPED",
                "mapping_basis": "PROVISIONAL_CATALOG_OVERLAY",
                "credential_id": "VOCATIONAL_NURSING_2025",
                "credential_title": "Vocational Nursing",
                "requirements_met": 0.0,
                "requirements_total": float(len(VOCATIONAL_NURSING_OVERLAY)),
                "progress_ratio": 0.0,
                "audit_status": "",
            }
    ...
    search_text = (
        candidates["credential_id"].fillna("") + " " + candidates["credential_title"].fillna("")
    ).str.upper()
    ...
    if candidates.empty:
        return {
            ...
            "mapping_basis": "KEYWORD_FALLBACK",
            "credential_id": "",
            "credential_title": "",
            "requirements_met": np.nan,
            "requirements_total": np.nan,
    ...
    candidates = candidates.merge(
        progress[["credential_id", "requirements_met", "requirements_total", "audit_status"]],
        on="credential_id", how="left"
    )
    candidates["requirements_met"] = pd.to_numeric(candidates["requirements_met"], errors="coerce").fillna(0)
    candidates["requirements_total"] = pd.to_numeric(candidates["requirements_total"], errors="coerce").replace(0, np.nan)
    candidates["progress_ratio"] = (candidates["requirements_met"] / candidates["requirements_total"]).fillna(0)
    candidates["hours_distance"] = ((candidates["program_hours"] - float(preferred_hours)).abs() if preferred_hours is not None else 0)
...
# Evaluate the catalog-sourced Vocational Nursing overlay directly from Livingston
# course history. This is intentionally local and provisional until the full audit
# requirements master is repaired and rerun.
passed_sets = (
    livingston_history[
        livingston_history["passed"].fillna("").str.upper().isin({"TRUE", "1", "YES", "Y"})
    ]
    .groupby("student_id")["course_code_normalized"]
    .agg(lambda values: set(v for v in values if v))
    .to_dict()
)

for idx in student_map.index[student_map["mapping_basis"].eq("PROVISIONAL_CATALOG_OVERLAY")]:
    sid = student_map.at[idx, "student_id"]
    passed = passed_sets.get(sid, set())
    met = 0
    for _, _, recommendation, _, rec_type in VOCATIONAL_NURSING_OVERLAY:
        if rec_type == "REQUIRED COURSE":
            met += int(recommendation in passed)
        else:
            met += int(("BIOL 1322" in passed) or ("FDNS 1345" in passed))
    total = len(VOCATIONAL_NURSING_OVERLAY)
    student_map.at[idx, "requirements_met"] = met
    student_map.at[idx, "requirements_total"] = total
    student_map.at[idx, "progress_ratio"] = met / total if total else 0
    student_map.at[idx, "audit_status"] = "COMPLETE" if met == total else "INCOMPLETE"
...
print("Mapping basis:")
print(student_map["mapping_basis"].replace("", "UNRESOLVED").value_counts(dropna=False).to_string())
print()
...
target_keys = set(
    mapped[
        ~mapped["mapping_basis"].eq("PROVISIONAL_CATALOG_OVERLAY")
    ][["student_id", "credential_id"]].itertuples(index=False, name=None)
)
...
# Add local Vocational Nursing requirements for the 19 VNSG students.
for student in mapped[mapped["mapping_basis"].eq("PROVISIONAL_CATALOG_OVERLAY")].itertuples(index=False):
    passed = passed_sets.get(student.student_id, set())
    for seq, semester, rec, hours, rec_type in VOCATIONAL_NURSING_OVERLAY:
        if rec_type == "REQUIRED COURSE":
            is_met = rec in passed
        else:
            is_met = ("BIOL 1322" in passed) or ("FDNS 1345" in passed)
        if is_met:
            continue
        recommendation_rows.append({
            "student_id": student.student_id,
            "credential_id": "VOCATIONAL_NURSING_2025",
            "requirement_id": f"VOCATIONAL_NURSING_2025__{seq:02d}",
            "sequence": float(seq),
            "semester_label": semester,
            "rule_type": "EXACT" if rec_type == "REQUIRED COURSE" else "ANY_N",
            "requirement_status": "UNMET",
            "group_name": semester,
            "source_requirement_text": rec,
            "required_options": rec,
            "recommendation": rec,
