...
def completion_academic_year(term_value: str) -> str:
    """
    Convert LSCO/Banner term codes to academic years.

    LSCO term suffixes observed in the audit:
      90 = Fall term          -> YYYY-(YYYY+1)
      95 = Late-fall session  -> YYYY-(YYYY+1)
      60 = Summer term        -> (YYYY-1)-YYYY
      15 = Winter/Spring mini -> (YYYY-1)-YYYY
    ...
    Already formatted academic-year labels are preserved.
    ...
    if any(token in lower for token in ("winter", "spring", "sp", "summer", "su")):
        return f"{year - 1}-{year}"
