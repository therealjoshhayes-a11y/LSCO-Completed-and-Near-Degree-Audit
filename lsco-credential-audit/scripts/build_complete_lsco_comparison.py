...
def find_latest_detected_files() -> tuple[Path, Path]:
    count_candidates = list(REPORTING_ROOT.glob("credential_completion_comparison_*/detected_counts_by_year_major.csv"))
    student_candidates = list(REPORTING_ROOT.glob("credential_completion_comparison_*/detected_student_completions.csv"))

    if not count_candidates:
        raise SystemExit("Could not find detected_counts_by_year_major.csv under data/processed/reporting.")
    if not student_candidates:
        raise SystemExit("Could not find detected_student_completions.csv under data/processed/reporting.")

    count_path = max(count_candidates, key=lambda p: p.stat().st_mtime)
    # Prefer student file from same folder as counts.
    same_folder_student = count_path.parent / "detected_student_completions.csv"
    student_path = same_folder_student if same_folder_student.exists() else max(student_candidates, key=lambda p: p.stat().st_mtime)
    return count_path, student_path
...
def main() -> None:
    detected_counts_path, student_path = find_latest_detected_files()
    print(f"Detected counts: {detected_counts_path}")
    print(f"Student detail:  {student_path}")

    detected = pd.read_csv(detected_counts_path, dtype=str, low_memory=False).fillna("")
    students = pd.read_csv(student_path, dtype=str, low_memory=False).fillna("")

    required_detected = {"completion_year", "credential_lineage", "major", "audit_detected_count"}
    missing = required_detected - set(detected.columns)
    if missing:
        raise SystemExit("Detected-count file is missing: " + ", ".join(sorted(missing)))

    detected["audit_detected_count"] = pd.to_numeric(detected["audit_detected_count"], errors="coerce").fillna(0).astype(int)
    unique_candidates = (
        detected[["credential_lineage", "major"]]
        .drop_duplicates()
        .sort_values(["major", "credential_lineage"])
        .reset_index(drop=True)
    )
    unique_candidates["candidate_text"] = (
        unique_candidates["major"].astype(str) + " " + unique_candidates["credential_lineage"].astype(str)
    )
    ...
    with pd.ExcelWriter(OUTPUT_WORKBOOK, engine="xlsxwriter") as writer:
        ...
        executive.merge_range(
            "A3:H3",
            "Published source: LSCO Degrees and Certificates Awarded by Program (2019-2020 through 2024-2025). Detected counts are locally generated and lineage-deduplicated.",
            subtitle_fmt,
        )
