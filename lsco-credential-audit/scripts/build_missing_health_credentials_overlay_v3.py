...
CORE_BUCKET_PATTERNS = [
    ...
    ("COMMUNICATION", re.compile(r"\bcommunication\s+core\b", re.I)),
    ("MATHEMATICS", re.compile(r"\bmathematics\s+core\b", re.I)),
    ("LIFE_AND_PHYSICAL_SCIENCES", re.compile(r"\blife\s+and\s+physical\s+sciences?\b", re.I)),
    ("SOCIAL_AND_BEHAVIORAL_SCIENCES", re.compile(r"\bsocial\s+and\s+behavioral\s+sciences?\b", re.I)),
]
...
def is_prerequisite_label(text: str) -> bool:
    return bool(PREREQ_RE.search(clean(text)))
...
def parse_curriculum_rows(
    ...
    for item in block.items:
        if item["kind"] == "paragraph":
            ...
            if is_prerequisite_label(text_value):
                current_semester = text_value
                in_prerequisites = True
                continue

            if SEMESTER_RE.search(text_value) or LEVEL_RE.search(text_value):
                current_semester = text_value
                in_prerequisites = False
                # Continue only when this is a pure section label. Course-bearing
                # paragraph rows still need to be parsed below.
                if not COURSE_RE.search(text_value.upper()):
                    continue

            # Some DOCX exports store course-bearing rows as paragraphs.
            paragraph_codes = []
            for rubric, number in COURSE_RE.findall(text_value.upper()):
                code = f"{rubric} {number}"
                if code not in paragraph_codes:
                    paragraph_codes.append(code)

            if paragraph_codes:
                option_groups, exact_courses = split_localized_or_groups(
                    text_value, paragraph_codes
                )
                    ...
                    diagnostics.append(
                        {
                            ...
                            "source_row_index": item["block_index"],
                            "raw_requirement_text": text_value,
                            "course_codes_found": " | ".join(paragraph_codes),
                ...
                for group in option_groups:
                    emit_requirement(
                        ...
                        raw_text=text_value,
                        table_index=-1,
                        row_index=item["block_index"],
                    )
                for course_code in exact_courses:
                    emit_requirement(
                        rule_type="EXACT",
                        course_codes_value=course_code,
                        credit_hours_value=inferred_hours(course_code),
                        raw_text=text_value,
                        table_index=-1,
                        row_index=item["block_index"],
                    )
        ...
        for row_index, cells in enumerate(rows):
            ...
            row_has_prereq = is_prerequisite_label(row_text)
            row_has_section = bool(
                SEMESTER_RE.search(row_text) or LEVEL_RE.search(row_text)
            )
    ...
    for year, targets in sorted(by_year.items()):
        ...
        for heading, metadata in targets:
            ...
            expected_auditable_hours = (
                float(metadata["expected_hours"]) - float(excluded_prerequisite_hours)
            )
            ...
            validation_rows.append(
                {
                    ...
                    "prerequisite_hours_excluded": excluded_prerequisite_hours,
            ...
            print(
                ...
                f"prereq_excluded={excluded_prerequisite_hours:>3} | "
                f"{status}"
            )
